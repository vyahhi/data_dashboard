plot_timeline = ->
   d3.json "/data/timeline.json", (activities) ->
      render = ->
         ...
         height = (if d3.select(window)[0][0].outerWidth >= 1000 then d3.select("#progress")[0][0].offsetHeight - 360 else width / 2)
         ...
         hlineG = svg.selectAll(".reference").data([
            perdayThreshold / 2
            perdayThreshold
            50
            100
